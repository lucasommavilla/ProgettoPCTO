...
function generaSelectAzienda($sqlAzienda){
  ...
  if ($sqlAzienda->num_rows > 1) {
    ...
    ?><option value="" disabled <?php if($_SESSION["aziendaSelezionata"]==-1) echo"selected";?>>Seleziona Azienda</option>
    <?php
    while ($row = $sqlAzienda->fetch_assoc()) {
      $nome=$row["denominazione_azienda"];
       $id=$row["id"];
      ?><option value="<?php echo $id;?>" <?php if($_SESSION["aziendaSelezionata"]==$id) echo"selected"?> ><?php echo $nome; ?></option>
      <?php
    }
  } else if ($sqlAzienda->num_rows == 1) {
      $row = $sqlAzienda->fetch_assoc();
      $nome=$row["denominazione_azienda"];
       $id=$row["id"];
       ?><option value="" disabled <?php echo"selected";?>><?php echo $nome; ?></option>
    <?php
      $_SESSION["aziendaSelezionata"] = $row["id"];
    }
