...
<html lang="en">
  <head>
  <title>Dettaglio Segretario</title>
...
<script>
    function mostraAlert() {
        ...
        var r = confirm('Sei sicuro di voler ELIMINARE il segretario '+nome.toUpperCase()+' '+cognome.toUpperCase()+'?');
        if (r == true) {
            document.getElementById("hidden_elimina").value = 3; //QUA 1
        }
    }
</script>
  </head>
  ...
  if ($id != -1) {
      ...
      $sql= "INSERT INTO `persona` (`datanascita`, `codicefiscale`, `residenza`, `indirizzo`, `cognome`, `nome`, `cellulare`, `telefono`, `cap`, `email`, `sesso`) VALUES (?,?,?,?,?,?,?,?,?,?,?);";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("sssssssssss", $_POST["datanascita"], $_POST["codicefiscale"], $_POST["residenza"], $_POST["indirizzo"], $_POST["cognome"], $_POST["nome"], $_POST["cellulare"], $_POST["telefono"], $_POST["cap"], $_POST["email"], $sesso);
      $stmt->execute();
      $last_id = $conn->insert_id;
      $sql="INSERT INTO `persona_ruolo` (`idruolo`, `idpersona`) VALUES ('3', '".$last_id."');";
      $result = $conn->query($sql);
      header("location:segretario-dettaglio.php?id=".$last_id); 
    }
  }
          ...
          echo '
            ...
            <p class="fs-5"><span data-feather="user"class="align-baseline"></span> Dati anagrafici</p>
              ...
              <div class="mb-3 row">
                  ...
                  <label class="col-sm-3 col-md-1 col-form-label">Codice Fiscale *</label>
                  <div class="col-sm-3 col-md-3">
                      <input type="text" maxlength="16" class="form-control" name="codicefiscale" value="'. $codicefiscale .'">
                  </div>
              </div>
...
<div class="container-fluid">
  ...
  <?php require("Include/menu.php");?>
    ...
    if(isset($_GET["id"])){
      ...
      <nav class="navbar">
        <div class="container-fluid">
        <p class="h2 mb-3"> Dettaglio del segretario '.htmlspecialchars($row["nome"]).' '. htmlspecialchars($row["cognome"]).'</p>
        </div>
      </nav>
      '; 
      }else{
        echo '<h1 class="h2 mb-3">Attenzione: Segretario inesistente</h1>';  
      }
      
    }else{
      echo '<h1 class="h2 mb-3">Aggiungi nuovo segretario</h1>';  
    }
    ?>
    ...
    if(isset($_GET["id"])){
      ...
      if(!isset($_POST["modifica"])){
        ...
        <div class="mb-4 clearfix">
                <div class="btn-toolbar float-start" role="toolbar" aria-label="Toolbar with button groups">
                  <div class="btn-group me-2" role="group" aria-label="First group">
                          <button type="button" onclick="location.href='segretario.php'" name="indietro" value="" class="btn btn-secondary">
                ...
                <input type="submit" onclick="mostraAlert()" name="elimina" value="Elimina Segretario" class="btn btn-danger float-end">
            </div>
            <?php
        ...
        echo'<div class="mb-4 clearfix">';
          echo'<input type="button" onclick="location.href=\'segretario-dettaglio.php?id='.$id.'\'" name="annullamodifiche" value="Annulla Modifiche" class="btn btn-secondary float-start">';
      ...
      $result = $conn->query($sql);
        echo'<div class="mb-4 clearfix">';
        echo'<input type="button" onclick="location.href=\'segretario.php\'" name="annullamodifiche" value="Annulla" class="btn btn-secondary float-start">';
        echo'<input type="submit" name="salva" value="Salva" class="btn btn-primary float-end">';
        echo '</div>';
        generaDettaglioModifica($result);
